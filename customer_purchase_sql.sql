...
round(avg(purchase_amount),2) as average_purchase_amount, sum(purchase_amount) as total_revenue from customer
...
select customer_segment, count(customer_id) as customer_count from customer_type
...
(
    ...
    rank() over(partition by category order by count(item_purchased) desc) as item_rank from customer
